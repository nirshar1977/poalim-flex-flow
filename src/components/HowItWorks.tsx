...
const HowItWorks: React.FC = () => {
  ...
  return (
    <section id="how-it-works" className="section-container bg-white border-b border-gray-200">
      <div className="text-center mb-12">
        <h2 className="text-3xl md:text-4xl font-bold text-poalim-navy mb-4">איך <span className="text-poalim-blue">פועלים פלקס</span> עובד?</h2>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {steps.map((step) => (
          <Card key={step.id} className="border-none hover:border-poalim-blue/20 hover:shadow-lg transition-all duration-300">
            <CardContent className="p-6 flex flex-col items-center text-center">
              <div className="bg-poalim-blue/10 p-4 rounded-full mb-4">
                {React.cloneElement(step.icon as React.ReactElement, { className: 'w-10 h-10 text-poalim-blue' })}
              </div>
              <div className="bg-poalim-blue text-white h-8 w-8 rounded-full flex items-center justify-center font-bold mb-4">
                {step.id}
              </div>
