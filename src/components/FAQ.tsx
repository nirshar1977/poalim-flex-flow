  ...
  const faqs = [
    {
      ...
      answer: "ניתן להשתמש בשירות עד 4 פעמים בשנה, בהתאם למסלול המשכנתא שלך ולהיסטוריית התשלומים."
    },
    {
  ...
  return (
    <section id="faq" className="section-container bg-poalim-gray">
      <div className="text-center mb-12">
        <h2 className="text-3xl md:text-4xl font-bold text-poalim-text mb-4">שאלות נפוצות</h2>
      ...
      <div className="max-w-3xl mx-auto">
        <Accordion type="single" collapsible className="bg-white rounded-xl overflow-hidden card-shadow">
          {faqs.map((faq, index) => (
            <AccordionItem key={index} value={`item-${index}`} className="border-b border-poalim-border">
              <AccordionTrigger className="px-6 py-4 hover:bg-poalim-light text-right font-medium text-poalim-text">
                {faq.question}
              </AccordionTrigger>
              <AccordionContent className="px-6 py-4 text-poalim-text/80">
                {faq.answer}
              </AccordionContent>
            </AccordionItem>
          ))}
